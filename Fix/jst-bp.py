...
X = minmax_scale(data)
Y = onehot_enc(actual)
c = 8, 15, 2 ##initialize network
X_train, X_test, y_train, y_test = train_test_split(X, Y, test_size=.1)
w, ep, mse , mseArray= bp_fit(c, X_train, y_train, 0.2, 1000, .1)
...
# fileOpen = open('spotify.csv', errors='ignore')
# readFile = csv.reader(fileOpen)
# data = list(readFile)
# actual = list()
# for i in range(len(data)):
...
# X = minmax_scale(data)
# Y = onehot_enc(actual)
# c = 14, 12, 2 ##initialize network
# X_train, X_test, y_train, y_test = train_test_split(X, Y, test_size=.1)
# w, ep, mse , mseArray= bp_fit(c, X_train, y_train, .1, 100, .01)